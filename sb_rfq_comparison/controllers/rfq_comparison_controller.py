            ...
            for line in record.order_line:
                if line.product_id.id not in product:
                    product[line.product_id.id] = {'product_name': line.product_id.name,
                                                    'records': {record.id: {'price': '{:,.2f}'.format(line.price_unit), 'uom': line.product_uom_id.name,'qty': '{:,.3f}'.format(line.product_qty), 'subtotal': '{:,.2f}'.format(line.product_qty*line.price_unit)}}}
                else:
                    product[line.product_id.id]['records'][record.id] = {'price': '{:,.2f}'.format(line.price_unit), 'uom': line.product_uom_id.name,
